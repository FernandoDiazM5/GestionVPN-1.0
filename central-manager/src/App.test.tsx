...
describe('Joinpoint Central',()=>{
  ...
  it('presenta el acceso central con contraseña y MFA',async()=>{render(<App/>);expect(await screen.findByRole('heading',{name:'Bienvenido'})).toBeInTheDocument();expect(screen.getByLabelText('Correo')).toHaveAttribute('autocomplete','username');expect(screen.getByLabelText('Contraseña')).toHaveAttribute('autocomplete','current-password');expect(screen.getByLabelText('Código de autenticación')).toHaveAttribute('autocomplete','one-time-code');});
  it('envía los tres factores y abre el resumen',async()=>{mocks.login.mockResolvedValue({id:'1',email:'admin@joinpoint.cloud',displayName:'Administrador'});const user=userEvent.setup();render(<App/>);await user.type(await screen.findByLabelText('Correo'),'admin@joinpoint.cloud');await user.type(screen.getByLabelText('Contraseña'),'una-clave-segura');await user.type(screen.getByLabelText('Código de autenticación'),'123456');await user.click(screen.getByRole('button',{name:'Ingresar de forma segura'}));await waitFor(()=>expect(mocks.login).toHaveBeenCalledWith('admin@joinpoint.cloud','una-clave-segura','123456'));expect(await screen.findByText('Estado de la plataforma')).toBeInTheDocument();});
});
