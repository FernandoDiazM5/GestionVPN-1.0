...
> - **Backlog restante (no aplicado):** M1 drift BD↔router (14 nodos en BD, 1 VRF en router; `ip_tunnel`/`node_number` obsoletos `10.0.2.x`/`10.10.250.x`/null) → purgar/re-provisionar; M2 unificar guardas a `req.account` y retirar `req.user` legacy; M5 normalizar `tunnel_assignments.tunnel_id` a clave canónica; M6 `apPollJob` SSH completo (12-14 cmds) cada 5 min/AP → poll ligero. B1 Dockerfile `USER` no-root; B2 nginx `server_name` explícito.
>
