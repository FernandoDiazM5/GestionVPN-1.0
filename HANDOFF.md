...
> - **Backlog restante (no aplicado):** M5 normalizar `tunnel_assignments.tunnel_id` a clave canónica; M6 `apPollJob` SSH completo (12-14 cmds) cada 5 min/AP → poll ligero. B1 Dockerfile `USER` no-root; B2 nginx `server_name` explícito. (Limpieza pendiente: duplicados/huérfanos del address-list YA existentes en el router — M3 solo evita NUEVOS.)
>
