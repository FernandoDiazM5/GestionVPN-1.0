...
> - **Backlog restante (no aplicado):** B1 Dockerfile `USER` no-root; B2 nginx `server_name` explícito (ambos tocan el despliegue activo → hacer en ventana). Limpieza de router pendiente: duplicados/huérfanos del address-list YA existentes (M3 solo evita NUEVOS) + `.rsc` de retorno del pool del VPS.
>
