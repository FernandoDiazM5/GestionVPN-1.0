...
| 🟡 RED en VPS | Aplicar runbook de migración `10.x` (fases del `migrate-mgmt-net.rsc`, incluida la nueva **FASE 2b** `/ip service set api/api-ssl address=` con los `10.x` — sin ella el login de la API se cuelga; ver §4.18 · corte final que elimina `VPN-WG-MGMT`) · cerrar puertos sobrantes en `ufw`. (Correo en prod: DO bloquea 25/465/587 → el usuario usa **puerto/relay alterno**; resuelto por su lado.) (En el router local ya se aplicó FASE 2b en vivo 2026-06-21. `scan:assign` ya NO es necesario en altas nuevas: la scan-IP se amarra al crear el workspace; úsalo solo para workspaces creados antes de este cambio.) |
