...
> - **Verificación:** `npm run build --prefix vpn-manager` OK; `npm run check:all` OK; `npm run test:all` = 337 backend + 66 frontend. Semgrep 1.166.0: 0 findings; el parser error del script UTF-16 se eliminó con el archivo, pero quedaron timeouts de fixpoint en archivos grandes (riesgo residual explícito). El commit pasó hooks de ESLint autofix, `node --check` y TypeScript. Pendiente push de `vps_prod` y deploy.
