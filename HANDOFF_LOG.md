...
> **Sesión 2026-07-10 (cont. 21) — 3 features (trustedNets, moderador-por-usuario, multi-workspace) + auditoría/limpieza de código muerto. Commiteadas a `dev`, SIN desplegar.** Tips: `dev` `54d765a` (trustedNets) → `00b23fe` (RBAC+cleanup) → commit de este handoff. **Salud:** `tsc` 0 · **347 tests backend (+9)** · 66 frontend · build OK · `audit:design` 0 errores.
> - **(1) `trustedNets` del bootstrap del Core** (`54d765a`): el bootstrap (cont. 19) ya blindaba equipos nuevos pero pasaba `trustedPublics:[]` hardcodeado → dejaba fuera la LAN local del admin. Nuevo campo `trustedNets` (CIDR, `coreRouter.ts` + `CoreRouterWizard` "Opciones avanzadas" + `settings.routes.js`) → `LIST-MGMT-TRUSTED` + allowlist api/api-ssl. Regla §4.36 ampliada. +1 test.
> - **(2) Moderador por USUARIO sin correo** (`00b23fe`): alta directa con `username`+contraseña → email sintético `<username>@local.app` (patrón `sessionBridge`, **cero migraciones**; el `UNIQUE(email)` da la unicidad). `lib/localAccount.js` (reexporta helpers de contracts + reservados admin/root/system), `utils/identity.ts`, `UsernameSchema`/`syntheticEmail`/`isSyntheticEmail` en `common.ts`, `CreateModeratorRequestSchema` email XOR username. `notifier`/`password-reset` saltan el sintético. `CreateModeratorModal` con toggle Directo/Invitación + credenciales one-time; listas muestran "sin correo" en vez del sintético; `ProfileTab` "Asociar correo". Reglas §4.39/§4.40. Verificado E2E (curl + navegador). Detalle de diseño: plan `federated-toasting-pine.md`.
> - **(3) Multi-workspace por usuario** (`00b23fe`): `workspace_members` ya era `UNIQUE(workspace_id,user_id)` → el schema soportaba multi-membresía; faltaba la sesión. **Backend:** `workspaceRepo.listMembershipsByUser`, `GET /account/workspaces`, `POST /account/switch-workspace` (re-firma JWT validando membresía, desactiva túnel saliente), invitar por username en `/team/invite` (resuelve server-side, in-app sin correo si sintético). **WG global** (`migrate:globalwg`: `UNIQUE(user_id)`): `memberWgRepo`/`mgmtIpRepo` por user; `mgmtAllowedIpsForUser` (unión de LAN públicas de todos sus ws); borrados condicionales (peer/IP sobreviven si quedan membresías; la cascada del DELETE moderador reasigna las filas de survivors a su otro ws por la FK). **Mangle por-usuario** (única física): `createSession` cierra ACTIVE global, `deactivateTunnel` no mata túnel de otro ws, `removeUserMangles(ids,{workspaceId})` scoped. **Frontend:** `WorkspaceSwitcher` en Sidebar (dropdown + badge de invitaciones + `reload` tras switch), `InvitePanel` toggle correo/usuario. Reglas §4.41/§4.42. Verificado E2E: invitación cruzada in-app, switch OWNER↔MEMBER, 403 a ws ajeno, expulsión preservando la cuenta.
> - **(4) Auditoría de código muerto** (en `00b23fe`): a pedido del usuario. Cross-ref de imports → **-10 archivos frontend huérfanos** (`NetworkDevicesModule/utils/{auth,scan,device}Service`, `ipValidation`, `useNodeSelection`; `NodeAccessPanel/utils/{nodeValidation,passwordGenerator}` = duplicados de `subnet.ts`/`password.ts`, `useNodePolling`; `SettingsModule/utils/settingsValidator`; `UserManagementPanel/components/AdminPeersManager`) + **-7 funciones backend sin invocación** (`getActiveByTunnel`, `getByWorkspace`, `softRemove`, `setPeerEnabled`, `findLegacyGlobalMangleIds`, `markVerified`, `clientCount`). **Fix real:** `/member/:id/wireguard` usaba `mgmtAllowedIpsFor` ws-scoped → cambiado a `mgmtAllowedIpsForUser` (el `.conf` que genera el moderador ahora une las LAN de todos los ws del miembro). Se DEJÓ: `db/rotateSecrets.js` (ops manual válido, sin cablear), exports cosméticos, docs raíz posiblemente superseded por `docs/`.

> **Sesión 2026-07-09 (cont. 20) — Incidente de seguridad: fuerza bruta al API del Core + mejora del bootstrap (redes de gestión de confianza).** Rama `dev` (sobre `69ba61c`, cambios SIN commitear al cierre). **Salud:** `tsc` frontend 0 · build prod OK · **339 tests backend (+1)** · `build:contracts` OK.
> - **Incidente (2026-07-08 ~23:5x):** el usuario reportó ataque al MikroTik core — el Log mostraba decenas de `login failure for user <admin/root/support/mkauth/superadmin/…> via api` desde **`45.198.224.18`** (una sola IP pública, diccionario de usuarios). **Causa raíz:** el API (:8728) estaba EXPUESTO a internet — el reset de config del Core del 2026-07-03 (cont. 16) borró la restricción `/ip service api address=` de la §4.18, re-abriendo el servicio. **Ninguna entrada era `logged in`** → no entraron (solo intentos).
> - **Respuesta (runbook dado al usuario, aplicado por él en Winbox):** restringir `/ip service api,api-ssl,winbox,ssh address=` a los planos de gestión `10.12/10.13/10.14.250.0/24` **+ su LAN local `192.168.18.0/24`** (o perdía Winbox); `disable telnet,ftp,www`; `address-list BLACKLIST` + drop de `45.198.224.18`; backup/export post-hardening. Confirmado por el usuario ("lo corregí y restringí a ciertas reglas").
> - **Mejora de código (este cambio, 4 archivos):** el bootstrap del Core (cont. 19) ya blinda equipos NUEVOS igual (disable ftp/telnet/www + allowlist api + drop WAN), pero `settings.routes.js` pasaba `trustedPublics: []` hardcodeado → un equipo blindado solo se administraba por el túnel `10.x`, dejando fuera la LAN local del admin. Nuevo campo **`trustedNets`** (`coreRouter.ts`, CIDR IPv4, máx 10) → wizard `CoreRouterWizard.tsx` (input en "Opciones avanzadas", coma-separado) → ruta lo pasa como `trustedPublics` → `applyBaseline` ya lo inyecta en `LIST-MGMT-TRUSTED` + allowlist api/api-ssl. Test nuevo en `coreBootstrap.test.js` (la red extra llega a ambos). Reglas: **§4.38** (nueva) + §4.36 y §5 actualizadas. **Sigue sin desplegar (pendiente cont. 19 de probar el bootstrap contra un CHR real).**

> **Sesión 2026-07-07 (cont. 19) — Feature: "Habilitar router como Servidor VPN" (bootstrap del Core desde Ajustes).** Rama `dev` = **`141ee3b`** (solo `dev`, NO en `main`, NO desplegado, NO probado aún contra un CHR real). **Salud:** `tsc` frontend 0 · eslint 0 errores · **338 tests backend (+10)** · 66 tests frontend · `audit:design` 0 errores. Skills: `handoff-keeper` (+ input del usuario con el `.rsc` de bootstrap `vpn_habil.rsc` y agentes `mikrotik-vpn-expert`/`api-contract` como referencia). Base: el `.rsc` es la ESPECIFICACIÓN, pero NO se pega — se reconstruye por la RouterOS API de forma idempotente, derivando de `mgmtNet.js`.
> - **Objetivo:** que el Administrador levante cualquier RouterOS/CHR en blanco (ej. CHR de DigitalOcean) desde **Ajustes**: ingresa IP + usuario + contraseña, el sistema valida, clasifica el equipo, aplica el andamiaje VPN y muestra un checklist de todo lo aplicado (igual que el alta de nodo). Si ya trabaja, lee los valores y muestra "conforme".
> - **Contratos** (`packages/contracts/src/coreRouter.ts`): `CoreRouterValidate/Bootstrap/StatusResponse` + `CoreBootstrapStep` (estado `skip` = ya presente, para la idempotencia). Reexport en `index.ts`, `build:contracts`.
> - **Backend** (`server/lib/coreBootstrap.js`): `readRouterState(api)` clasifica en **blank/ours/partial/foreign** (lee WG mgmt, sstp-server, api allowlist, filter, VRFs, peers, identity/resource). `applyBaseline(api, opts, pushStep)` monta **idempotente** (leer-antes-de-crear, como `addRouteOnce`): interface-lists, 3 interfaces WG de gestión + IPs gateway `.1`, pool SSTP, perfil PPP, sstp-server, address-lists (`LIST-MGMT-TRUSTED`/`vpn-activa`), firewall filter+forward completo, NAT del andamiaje (bypass admin + masquerade SSTP), allowlist api/api-ssl, DNS/identidad. Todo **derivado de `mgmtNet`** (0 hardcode del `.rsc`). `detectBackendOrigin(api)` lee `/ip firewall connection` para el anti-bloqueo.
> - **Rutas** (`server/routes/settings.routes.js`, solo `platform_admin`): `POST /settings/core-router/validate` (conecta con creds del body SIN guardar, clasifica), `POST .../bootstrap` (aplica + persiste `MT_*`/`server_public_ip`/`sstp_port` SOLO tras éxito; `409 CORE_DEVICE_CHANGE` si ya hay otro router y falta `confirmSwitch`), `GET .../status` (panel "conforme" con las creds guardadas). `validate` devuelve **400** (no 503) para mostrar el error inline sin disparar el overlay global.
> - **Frontend** (`CoreRouterWizard.tsx` en el módulo Ajustes): asistente IP pública (1er campo) → usuario/contraseña → validar → badge de estado + valores detectados → habilitar → checklist (✓ aplicado / = ya presente / ! diferido). Panel verde "conforme" con re-verificar si ya trabaja. Confirmación de cambio de equipo. 100% primitivas del design system (espejo de `ScanModeToggle`).
> - **Anti-bloqueo (§4.36/§4.18):** la regla `drop WAN input` y la restricción de `/ip service api` solo se aplican si el origen del backend está garantizado en trusted (detectado por `detectBackendOrigin`, o conectando por el plano `10.x`); si no, se **omiten con `warn`** — nunca se deja sin acceso a un CHR recién levantado. La allowlist de la API se toca solo por **UNIÓN** (nunca quita el origen vivo).
> - **Corrección del usuario a mitad de camino:** el bootstrap NO debe asumir/gestionar la **IP pública/WAN** (la provee la nube o el operador la habilita aparte con sus propias configs). Se quitó: detección de interfaz WAN, regla `src-nat NAT IP Publica`, miembro de `LIST-WAN` (solo se crea la lista, inerte). `server_public_ip` solo se **pre-rellena si está vacío** (nunca pisa lo del operador). Regla §4.37.
> - **Decisión de alcance:** los peers de gestión (admin/members/VPS) siguen por los flujos existentes (invitación, recuperación WG self-service). Sembrar el peer WG del admin en el bootstrap = **fase 2** (chicken-and-egg con la clave del dispositivo del admin). Sin migración de BD.
> - **Pendiente:** probar contra un CHR/VPS real (nuevo pendiente §7); FF a `main` + desplegar cuando se valide.

---

> **Sesión 2026-07-05 (cont. 18) — 4 bugs de prod: MEMBER deslogueado en cadena + escaneo 0 en workspace nuevo + autosync wg0 que nunca funcionó del todo + workspace pre-Opción C sin scan-IP. DESPLEGADO y verificado.** Rama `dev`=`main`=**`aa1864a`** (`87488fb`+`aa1864a`, pusheados a ambas). **Salud:** `tsc` 0 · **316 tests backend** (+2: `wg0Reconcile`) · pre-commit OK. **Verificado en vivo en el VPS** (Fases 1-5): `ip route get 192.168.30.1 → dev wg0` (antes `eth0`), diagnose sin collation, reconcile re-sembró OMAR al arrancar. Skills: `handoff-keeper`.
> - **Bug 1 — MEMBER deslogueado en cadena al activar un túnel compartido.** Síntoma: activar → 403 en `/api/node/history/add` → "Token expirado" → logout → re-login → repite. **Causa raíz doble:** (a) `apiClient.apiFetch` trataba **TODO** 401/403 fuera de `/api/auth/` como sesión expirada (`auth_expired`→logout); (b) `/node/history/add` tenía `requireOperator` → 403 de PERMISOS a un MEMBER. El frontend confundía el 403 de permisos con expiración. **Fix:** apiClient solo desloguea en 403 si el body trae `logout:true` (lo marca `auth.middleware` en token expirado real); 401 sí siempre. Quitado `requireOperator` de `/history/add` (append-only, gate real = `nodeBelongsToRequester` por workspace). Test H1 movido: history/add ya no es "mutación por rol" sino "gate por workspace" (404 nodo ajeno / 200 nodo propio). Regla nueva §4.32.
> - **Bug 2 — escaneo daba "254 IPs, 0 encontrados" en el workspace nuevo (SoporteHousenet/TorreOMAR).** **Causa raíz = routing del VPS, NO la mangle:** `ip route get 192.168.30.1 → dev eth0` (salía por internet). El `wg0` del VPS no tenía `192.168.30.0/24`. **Dos fallos del autosync (§4.27):** (i) el watcher `wg0-autosync.sh` hacía solo `wg syncconf`, que actualiza AllowedIPs pero **NO crea la ruta de kernel** → las `142.x` estaban en AllowedIPs pero sin `ip route` (huérfanas); (ii) el hook era event-driven puro (solo al provisionar) → OMAR, creado antes del autosync (cont.10), nunca registró su intención y no había recuperación. **Fix:** (i) el `.sh` ahora reconcilia rutas con `ip route replace` en cada corrida (siempre, aunque el AllowedIPs no cambie); (ii) nuevo `lib/wg0Reconcile.reconcileOnStartup()` (enganchado en `index.js` tras los jobs): al arrancar el backend re-siembra la intención con TODAS las LAN de `nodes` (idempotente, best-effort, no-op fuera del VPS) → recupera cualquier LAN perdida sin entrar al VPS. Si el dir `/opt/wg0-autosync` no es escribible por uid 1001 → ahora `log.error` VISIBLE (era la causa silenciosa; se cura con `chown -R 1001:1001`). Reglas nuevas §4.33/§4.34.
> - **Bug 3 (colateral) — `npm run diagnose` reventaba `workspace_scan_ip` con "Illegal mix of collations (uca1400_ai_ci vs general_ci)".** MariaDB 11.4+ crea tablas nuevas en `utf8mb4_uca1400_ai_ci` pero `migrateScanIp` forzó esa tabla a `general_ci` → el JOIN con `workspaces` explota. **NO era la causa del escaneo 0** (el hot path `getScanIpForWorkspace` usa `WHERE = ?`, sin JOIN). **Fix:** `COLLATE utf8mb4_general_ci` explícito en AMBOS lados del JOIN (`scanIpRepo.list()` + `diagnostics.js`). Robusto ante cualquier combinación. Regla nueva §4.35.
> - **Decisión de arquitectura (pools del wg0):** el usuario propuso meter superset RFC1918 (`10/8`, `172.16/12`, `192.168/16`) en el AllowedIPs del wg0. **Descartado `10/8` y `172.16/12`** con datos de `ip -br addr`: el VPS usa `10.48.0.0/16`+`10.124.0.0/20` (VPC DO) y `172.17/172.18` (Docker) → el superset los secuestraría. **Solo `192.168.0.0/16` es seguro** (el VPS no usa 192.168.x). Con el autosync arreglado, el superset ya no es necesario: cada LAN entra sola. Regla §4.34.
> - **Bug 4 — tras arreglar el wg0, el escaneo de OMAR SEGUÍA en 0: el workspace no tenía scan-IP.** En Winbox solo aparecía la mangle `ACCESO-USER` (nunca la `SCAN-WS`). TorreOMAR vive en "Soporte FIWIS" (`b188ec84`), workspace creado ANTES de la Opción C → sin fila en `workspace_scan_ip` → `scanMangleSync.onTunnelActivated` hacía `resolveForWorkspace → null → return` (no-op silencioso) → sin mangle → el router no marcaba el tráfico del scan al VRF. **Fix `aa1864a`:** en modo `vps`, si falta la scan-IP al activar el túnel se asigna del pool al vuelo (`scanIpRepo.allocate`, idempotente); en modo `local` sigue no-op (la IP es global). 318 tests (+3). **Verificado en prod:** log `scan-IP auto-asignada (10.11.252.3)` + mangle `SCAN-WS-b188ec845434` visible en Winbox. Con esto muere el runbook manual `scan:assign` para workspaces viejos.
> - **Despliegue:** commits `87488fb` (9 archivos: apiClient, history.routes+test, scanIpRepo, diagnostics, wg0-autosync.sh, wg0Reconcile+test, index.js) y `aa1864a` (scanMangleSync+test) → push `origin/dev`+FF `origin/main`. VPS: `git reset --hard origin/main` + `cp wg0-autosync.sh /usr/local/sbin/` + `chown -R 1001:1001 /opt/wg0-autosync` + `up -d --build` (×2) + `systemctl start wg0-autosync.service`. Sin migraciones. Fuera del commit lo ajeno (`HANDOFF*`, `docs/`, `.claude/`).

---

> **Sesión 2026-07-04 (cont. 17) — Auditoría visual UI profunda (agente `frontend-dev`) + pasada de consistencia del sistema de diseño + Spinner SVG canónico. DESPLEGADO en prod.** Rama `dev`=`main`=**`11bb27f`** (pusheado a ambas; **desplegado y verificado en el VPS el 2026-07-04** — arrastró todo el rango pendiente cont. 13→17). **Salud:** `tsc` 0 · **66 tests frontend** · **build prod OK** · `audit:design` **0 errores** (total 32→21; DS07/DS08/DS09 en 0) · verificado en preview (consola limpia, CSS generado confirmado). Skills/agentes: `find-skills`, `frontend-dev`, `handoff-keeper`.
...
> - **Ajuste post-auditoría (feedback del usuario):** los botones "Nuevo Moderador" (x2) y "Nuevo Nodo" salían **verdes** (`btn-success`); el usuario pidió que **predomine el índigo**. Corregidos a `btn-primary` — **regla: la acción primaria de CREAR es índigo, el verde queda para activar/confirmar/guardar** (verificado en preview: `rgb(79,70,229)` = indigo-600). Verdes legítimos intactos: Guardar N dispositivos, Activar túnel, Reanudar.
> - **Despliegue:** commit `11bb27f` (54 archivos) verificado (tsc/build/tests/audit) → push a `origin/dev` y FF a `origin/main` → **VPS actualizado** (`git reset --hard origin/main` + `up -d --build`). Sin migraciones nuevas (solo frontend). Se dejó FUERA del commit lo pre-existente ajeno a la pasada (`ARQUITECTURA.md`, `docs/`, `.claude/`).
> - **Pendiente del informe (no urgente):** footers a mano de modales NodeAccessPanel → `.modal-footer` · `.th-grid` para headers de tablas-grid · sticky de columna acciones en UsersTable · reglas candidatas DS10–DS18 (duration inexistente, `<th` sin `.th-cell`, texto/bordes sin par dark, `!bg-` sobre clases del sistema, `rounded-3xl` fuera de Auth) · decidir color de los 2 "Generar Script" (siguen verdes).
