    ...
    <div className="bg-white dark:bg-slate-800/40 border border-slate-200 dark:border-slate-700/50 rounded-xl p-4 shadow-sm">
      ...
      {antennaStats.ifaceDetails.map((ifc, i) => (
        <div key={i} className="mb-2 last:mb-0">
          <p className="text-2xs font-bold text-slate-500 dark:text-slate-400 uppercase mb-1">{ifc.ifname} {ifc.hwaddr && <span className="font-mono font-normal">{ifc.hwaddr}</span>}</p>
