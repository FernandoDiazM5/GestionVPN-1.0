...
export default function MikrowispCatalogs() {
  ...
  return <div className="space-y-3 rounded-xl border border-sky-200 bg-sky-50 p-3 dark:border-sky-800/60 dark:bg-sky-500/10">
    ...
    {loading ? <p className="flex items-center text-xs text-sky-700 dark:text-sky-300"><Loader2 className="mr-2 h-4 w-4 animate-spin" />Consultando catálogos…</p> : <div>{catalogs.map(catalog => <div key={catalog.type} className="rounded-lg border border-sky-100 bg-white p-3 dark:border-sky-900 dark:bg-slate-900">
