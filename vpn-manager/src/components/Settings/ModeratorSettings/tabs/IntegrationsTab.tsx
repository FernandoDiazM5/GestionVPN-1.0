...
import { Bot, BrainCircuit, CheckCircle2, ChevronDown, Database, ExternalLink, KeyRound, Loader2, Mail, RefreshCw, ShieldCheck, Trash2, TriangleAlert } from 'lucide-react';
...
import MikrowispGuideAdmin from './MikrowispGuideAdmin';
...
function getIntegrationGuide(provider: IntegrationProvider, scope: 'workspace' | 'platform'): IntegrationGuide {
  ...
  const guides: Record<IntegrationProvider, IntegrationGuide> = {
    ...
    MIKROWISP: { title: 'Conectar MikroWisp en 3 pasos', portalLabel: 'Ver documentación', portalUrl: 'https://docs.mikrosystem.net/reference/introducci%C3%B3n', steps: [
      { title: 'Obtén un token API', description: 'Genera o copia el token desde tu sistema MikroWisp.' },
      { title: 'Elige un cliente de prueba', description: 'Busca un ID de cliente existente para validar una consulta exacta.' },
      { title: 'Completa y valida', description: 'Ingresa el dominio HTTPS, token e ID. Joinpoint no ejecutará altas, cambios, cortes ni pagos.' },
    ], note: 'La credencial se cifra y las respuestas se filtran antes de usarse. No se guardan usuarios PPP, contraseñas ni secretos.' },
