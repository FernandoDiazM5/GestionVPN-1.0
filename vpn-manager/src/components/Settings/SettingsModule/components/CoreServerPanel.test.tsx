...
const mocks = vi.hoisted(() => ({ status: vi.fn(), health: vi.fn(), backupNow: vi.fn() }));
...
describe('CoreServerPanel', () => {
  ...
  it('muestra el estado del servidor existente y la política de respaldo', async () => {
    ...
    expect(screen.getByText(/Envía un .backup cifrado/)).toBeInTheDocument();
    expect(screen.queryByText('Asistente de configuración')).not.toBeInTheDocument();
  });

  it('no presenta acciones para preparar o reemplazar el Core', async () => {
    render(<CoreServerPanel {...baseProps} />);
    await screen.findByText('GW-VPN-CORE-ISP');
    expect(screen.queryByText('Preparar un servidor nuevo')).not.toBeInTheDocument();
    expect(screen.queryByText('Reemplazar el Core actual')).not.toBeInTheDocument();
  });
