        ...
        {isEmpty ? (
          ...
          <>
              ...
              <div style={{
                ...
                {columns.cols.map(({ ap, cpes }) => (
                  <div key={ap.id} style={{
                    ...
                    <ApCard device={ap} cpeCount={cpes.length} />

                    {cpes.length > 0 && (
                      <>
                        {/* vertical connector from AP down */}
                        <div style={{
                          width: 2, height: 24,
                          background: 'linear-gradient(to bottom, rgba(99,102,241,0.6), rgba(167,139,250,0.5))',
                          borderRadius: 2,
                        }} />
            ...
            {columns.orphans.length > 0 && (
              <div style={{ marginTop: columns.cols.length > 0 ? 36 : 0 }}>
                <div style={{
                  fontSize: 10, color: '#475569', textTransform: 'uppercase',
                  letterSpacing: 1.2, fontWeight: 700, marginBottom: 14,
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 12 }}>
                  {columns.orphans.map(cpe => (
                    <CpeCard key={cpe.id} device={cpe} />
                  ))}
                </div>
              </div>
            )}

            {/* APs with no CPEs + orphan APsonly section — already rendered in columns */}
            {aps.length === 0 && stas.length > 0 && (
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 12 }}>
                {stas.map(cpe => <CpeCard key={cpe.id} device={cpe} />)}
              </div>
            )}
          </>
        )}
