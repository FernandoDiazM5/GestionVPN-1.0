    ...
    render: (dev) => {
      ...
      return (
        <div className="min-w-0">
          {ssid && (
            <span className="block break-words font-mono text-2xs leading-4 text-slate-600" title={ssid}>{ssid}</span>
          )}
          {parentAp && parentAp !== ssid && (
            <span className="block break-words text-3xs leading-4 text-violet-500" title={`AP: ${parentAp}`}>{parentAp}</span>
          )}
        </div>
      );
    },
