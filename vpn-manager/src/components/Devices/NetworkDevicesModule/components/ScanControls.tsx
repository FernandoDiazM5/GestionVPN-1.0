...
import type { NodeSshCredsStatus } from '../hooks/useNodeSshCredentials';
...
interface ScanControlsProps {
  ...
  credentialsStatus: NodeSshCredsStatus;
...
function ScanControlsImpl({
  ...
  nodeSshCreds, credentialsStatus, credentialsError, savedCredentialCount, onReloadCredentials,
  effectiveLan, canScan, isScanning, onScan,
}: ScanControlsProps) {
  return (
    <>
      ...
      {isTunnelActive && activeNode && (
        <div className="border-t border-slate-100 pt-3 mt-1 flex flex-wrap items-center gap-2">
          <div className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs border ${credentialsStatus === 'error' || credentialsStatus === 'forbidden' ? 'bg-rose-50 border-rose-200 text-rose-700 dark:bg-rose-500/10 dark:border-rose-500/30 dark:text-rose-400' : nodeSshCreds.length > 0 ? 'bg-amber-50 border-amber-200 text-amber-700 dark:bg-amber-500/10 dark:border-amber-500/30 dark:text-amber-400' : 'bg-slate-50 border-slate-200 text-slate-500 dark:bg-slate-800/60 dark:border-slate-700 dark:text-slate-400'}`}>
            <KeyRound className="w-3.5 h-3.5 shrink-0" />
            {credentialsStatus === 'loading'
              ? <span className="flex items-center gap-1.5"><Loader2 className="w-3 h-3 motion-safe:animate-spin" /> Consultando credenciales SSH…</span>
              : credentialsStatus === 'forbidden'
                ? <span>No puedes consultar las credenciales del nodo con este rol</span>
          ...
          {(credentialsStatus === 'error' || credentialsStatus === 'forbidden') && (
            <button type="button" onClick={onReloadCredentials} title={credentialsError} className="text-xs font-semibold text-indigo-600 hover:text-indigo-700 dark:text-indigo-400">
              Reintentar
            </button>
          )}
          {savedCredentialCount > 0 && (
            <span className="px-3 py-1.5 rounded-lg text-xs border bg-emerald-50 border-emerald-200 text-emerald-700 dark:bg-emerald-500/10 dark:border-emerald-500/30 dark:text-emerald-400">
              {savedCredentialCount} equipo{savedCredentialCount > 1 ? 's' : ''} con credencial propia
            </span>
          )}
        </div>
      )}
