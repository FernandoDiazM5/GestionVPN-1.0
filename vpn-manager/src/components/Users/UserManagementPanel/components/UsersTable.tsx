...
export default function UsersTable({
  ...
  return (
    <div className="card overflow-hidden border border-slate-200">
                    ...
                    {isEditing ? (
                      <div className="flex items-center gap-1.5">
                        <input autoFocus value={editingPeerName} onChange={e => onChangeEditName(e.target.value)}
                          ...
                          className="flex-1 px-2 py-1 text-xs border border-indigo-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-300 font-semibold max-w-[160px] dark:bg-slate-800 dark:border-indigo-500/50 dark:text-slate-100" />
                      ...
                      <div className="flex items-center gap-1.5 group/name">
                        <span className={`font-semibold text-xs ${peer.active ? 'text-slate-800 dark:text-slate-100' : 'text-slate-400'}`}>{peer.name}</span>
      ...
      {/* Footer: totales */}
      {peers.length > 0 && (
        <div className="px-6 py-3 border-t border-slate-100 bg-slate-50 text-xs text-slate-500 dark:border-slate-800 dark:bg-slate-800/40 dark:text-slate-400">
