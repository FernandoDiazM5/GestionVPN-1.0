    ...
    <div className="card p-4">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          <span className="text-sm font-semibold text-slate-600 dark:text-slate-300">Acceso administrador</span>
          {adminPeers.length > 0 && (
        ...
        <div className="flex items-center gap-1.5">
          ...
          <button onClick={onAddAdmin}
            className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-xs font-bold bg-indigo-50 text-indigo-600 hover:bg-indigo-100 border border-indigo-200 transition-colors dark:bg-indigo-500/15 dark:text-indigo-300 dark:border-indigo-500/30 dark:hover:bg-indigo-500/25">
            <UserPlus className="w-3 h-3" /><span>Nuevo</span>
          </button>
          {adminPeers.length > 0 && (
            <button onClick={() => setPeersExpanded(!peersExpanded)}
              className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-xs font-semibold text-slate-500 hover:bg-slate-100 border border-slate-200 transition-colors dark:text-slate-400 dark:hover:bg-slate-800 dark:border-slate-700">
        ...
        <div className="flex items-center gap-2 mt-3 overflow-x-auto pb-2 scrollbar-thin">
          ...
          {adminPeers.map(peer => {
            ...
            return (
              <div
                key={peer.id}
                className={`flex items-center gap-2 px-3 py-2 rounded-xl text-xs font-semibold border border-slate-200 dark:border-slate-700 transition-all shrink-0
                  ${peer.active ? 'bg-white text-slate-700 dark:bg-slate-800 dark:text-slate-200' : 'bg-slate-50 text-slate-400 dark:bg-slate-800/50 dark:text-slate-500'}`}
              >
                {color
        ...
        <div className="mt-4 space-y-3">
          ...
          <div className="flex items-center gap-2 pb-3 border-b border-slate-100">
            ...
            <input value={serverEndpointIP}
              ...
              className="flex-1 px-2.5 py-1.5 text-xs border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-300 font-mono dark:bg-slate-800 dark:border-slate-700 dark:text-slate-100" />
            <span className="text-slate-400 font-bold text-xs">:</span>
            <input value={serverListenPort} onChange={e => setServerListenPort(e.target.value)}
              placeholder="Puerto"
              className="w-20 px-2.5 py-1.5 text-xs border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-300 font-mono dark:bg-slate-800 dark:border-slate-700 dark:text-slate-100" />
          </div>
          ...
          {adminPeers.map(peer => {
            ...
            return (
              <div key={peer.id} className="rounded-xl border border-slate-200 dark:border-slate-700 overflow-hidden">
                <div className={`flex items-center gap-3 px-3 py-2.5 ${peer.active ? 'bg-white dark:bg-slate-800' : 'bg-slate-50 dark:bg-slate-800/50'}`}>
                  ...
                  <button onClick={() => onCopyConfig(peer)}
                    className={`flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-2xs font-bold transition-colors shrink-0
                      ${copiedPeerId === peer.id ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-500/15 dark:text-emerald-400' : 'bg-slate-100 text-slate-600 hover:bg-emerald-50 hover:text-emerald-700 border border-slate-200 dark:bg-slate-700 dark:text-slate-300 dark:border-slate-600 dark:hover:bg-emerald-500/15 dark:hover:text-emerald-400'}`}>
                ...
                {/* Color picker */}
                {showPicker && (
                  <div className="flex items-center gap-2 px-3 py-2 bg-slate-50 border-t border-slate-100 dark:bg-slate-800/50 dark:border-slate-700">
