              ...
              <table className="w-full text-left">
                <thead>
                  <tr className="bg-slate-50 border-b border-slate-100">
                    <th className="px-4 py-2.5 text-[10px] font-bold text-slate-400 uppercase tracking-wider">Nombre</th>
                    <th className="px-4 py-2.5 text-[10px] font-bold text-slate-400 uppercase tracking-wider hidden sm:table-cell">IP / MAC</th>
                    <th className="px-4 py-2.5 text-[10px] font-bold text-slate-400 uppercase tracking-wider hidden md:table-cell">Modelo · Firmware</th>
                    <th className="px-4 py-2.5 text-[10px] font-bold text-slate-400 uppercase tracking-wider">Tipo</th>
                    <th className="px-4 py-2.5 text-[10px] font-bold text-slate-400 uppercase tracking-wider text-center">Acción</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-100">
                  {scanResults.map((dev) => {
                    ...
                    return (
                      <tr key={dev.mac} className="hover:bg-indigo-50/40 transition-colors">
                        <td className="px-4 py-3">
                          <p className="font-semibold text-sm text-slate-700">{dev.name}</p>
                          {dev.essid && <p className="text-[10px] text-slate-400 font-mono">{dev.essid}</p>}
                        </td>
                        <td className="px-4 py-3 hidden sm:table-cell">
                          <p className="font-mono text-xs text-slate-600">{dev.ip}</p>
                          <p className="font-mono text-[10px] text-slate-400">{dev.mac}</p>
                        </td>
                        <td className="px-4 py-3 hidden md:table-cell">
                          <p className="text-xs text-slate-600">{dev.model}</p>
                          <span className="font-mono text-[10px] text-slate-400 bg-slate-100 px-1.5 py-0.5 rounded">{dev.firmware}</span>
                        </td>
                        <td className="px-4 py-3">
                          <span className={`text-[10px] font-bold px-2 py-0.5 rounded-md ${dev.role === 'ap' ? 'bg-indigo-100 text-indigo-700' : 'bg-violet-100 text-violet-700'}`}>
                            {dev.role === 'ap' ? 'AP' : 'CPE'}
                          </span>
                        </td>
                          ...
                          {selectedNode && (
                            ...
                            >
                              {already
                                ...
                                : <><PlusCircle className="w-3.5 h-3.5" /><span>Guardar</span></>}
                            </button>
                          )}
                        </td>
                      </tr>
