    ...
    <div className="card p-5 border border-slate-200 dark:border-slate-800 space-y-4">
      ...
      <form onSubmit={submit} className="flex flex-col sm:flex-row gap-2">
        <div className="relative sm:w-56">
          <label htmlFor="team-invite-name" className="sr-only">Nombre del invitado</label>
          <User className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-500 dark:text-slate-400" />
          <input
            id="team-invite-name" name="name" type="text" autoComplete="name" maxLength={120}
        ...
        <div className="relative flex-1">
          <label htmlFor="team-invite-email" className="sr-only">Correo electrónico del invitado</label>
          <Mail className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-500 dark:text-slate-400" />
          <input
            id="team-invite-email" name="email" type="email" required autoComplete="email" maxLength={255}
      ...
      {error && <p role="alert" aria-live="assertive" className="text-xs text-rose-600 dark:text-rose-400 font-medium">{error}</p>}
      {devHint && <p role="status" aria-live="polite" className="text-xs text-amber-600 dark:text-amber-400">{devHint}</p>}

      {/* Invitaciones pendientes */}
      {invitations.length > 0 && (
        <div className="pt-3 border-t border-slate-100 dark:border-slate-800 space-y-2">
          <p className="text-2xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-500">Pendientes</p>
          {invitations.map(inv => (
            <div key={inv.id} className="flex items-center gap-3 px-3 py-2 rounded-lg bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-700">
              ...
              <button type="button" onClick={() => onRevoke(inv.id)} title="Revocar invitación" aria-label="Revocar invitación"
