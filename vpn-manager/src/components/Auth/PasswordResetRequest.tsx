          ...
          <div className="px-8 py-8 -mt-4 relative">
            {sent ? (
              ...
              <form onSubmit={submit} className="space-y-4">
                ...
                {error && <p id="password-reset-request-error" role="alert" aria-live="assertive" className="text-sm text-rose-600 dark:text-rose-400 font-medium bg-rose-50 dark:bg-rose-500/10 border border-rose-100 dark:border-rose-500/30 rounded-xl px-3 py-2">{error}</p>}
                <div>
                  <label htmlFor="password-reset-email" className="block text-xs font-semibold text-slate-500 mb-2">Correo electrónico</label>
                  <div className="relative">
                    <Mail className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-500 dark:text-slate-400" />
                    <input id="password-reset-email" name="email" type="email" required autoComplete="email" maxLength={255}
                      ...
                      autoFocus className="input-field pl-10" />
                  </div>
                </div>
