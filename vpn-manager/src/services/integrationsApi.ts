...
export interface TelegramBulkJob { id: string; groupId: string; status: 'PENDING' | 'RUNNING' | 'PAUSED' | 'COMPLETED'; totalClients: number; existing: number; pending: number; created: number; skipped: number; failed: number; startedAt: number | null; finishedAt: number | null; createdAt: number; updatedAt: number }
