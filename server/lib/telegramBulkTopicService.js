...
const BASE_DELAY_MS = Math.max(1500, Number(process.env.TELEGRAM_TOPIC_BULK_DELAY_MS) || 1500);
    ...
    try {
      ...
      if (!created.ok) {
        ...
        if (created.status === 429) {
          await pause(job.workspace_id, job.group_id, jobId);
          await sleep(Math.max(BASE_DELAY_MS, Number(created.retryAfter || 5) * 1000));
          return;
        }
