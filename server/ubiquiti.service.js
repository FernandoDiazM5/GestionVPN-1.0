...
const trySshCredentials = async (ip, credentialsArray) => {
    ...
    for (const cred of credentialsArray) {
        ...
        try {
            // Tiempos ajustados (12s) para asegurar que equipos lentos logren negociar SSH
            const output = await sshExec(ip, 22, cred.user, cred.pass, 'mca-status', 12000, 8000);
            ...
            continue; // Falla la clave, intentar con la siguiente
        }
    }
    return null; // Ninguna funcionó
};

module.exports = { IPV4_REGEX, CIDR_REGEX, getSubnetHosts, probeUbiquiti, sshExec, parseAirOSStats, trySshCredentials };