...
router.post('/mikrowisp/telegram-forums/:groupId/reconcile', validate({ params: GroupParamsSchema, body: z.object({}).strict() }), asyncHandler(async (req, res) => sendOk(res, await forums.reconcileGroup(req.account.workspace_id, req.account.sub, req.params.groupId))));
...
router.post('/mikrowisp/telegram-forums/:groupId/topics/:topicId/delete', validate({ params: TopicParamsSchema, body: z.object({ confirm: z.literal(true) }).strict() }), asyncHandler(async (req, res) => sendOk(res, { topic: await forums.deleteTopic(req.account.workspace_id, req.account.sub, req.params.groupId, req.params.topicId) })));
