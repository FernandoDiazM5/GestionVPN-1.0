...
async function listForUser({ workspaceId, userId, type, limit = 20 }, runQuery = query) {
  const params = [workspaceId, userId];
  let typeSql = '';
  if (type) { typeSql = ' AND analysis_type = ?'; params.push(type); }
  params.push(Math.min(50, Math.max(1, Number(limit) || 20)));
  const rows = await runQuery(
    `SELECT uuid, analysis_type, status, summary_json, model, total_tokens, created_at
       FROM ai_analysis_runs WHERE workspace_id = ? AND user_id = ?${typeSql}
...
async function getForUser({ workspaceId, userId, uuid }, runQuery = query) {
  const rows = await runQuery(
    `SELECT uuid, analysis_type, status, summary_json, scope_json, model,
            input_tokens, output_tokens, total_tokens, created_at
       FROM ai_analysis_runs WHERE uuid = ? AND workspace_id = ? AND user_id = ? LIMIT 1`,
...
async function purgeOlderThan(cutoffMs, runQuery = query) {
  const result = await runQuery(
    `DELETE FROM ai_analysis_runs
      WHERE created_at < ? AND status IN ('SUCCEEDED', 'FAILED', 'REJECTED')`,
    [cutoffMs]
  );
