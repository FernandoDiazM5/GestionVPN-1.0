...
async function recordFailure({ userId, ip, now = Date.now() }) {
  return withTransaction(async (tx) => {
    ...
    await tx.query(`UPDATE account_login_security SET failures_15m=?,window_15m_started_at=?,
      failures_24h=?,window_24h_started_at=?,locked_until=?,lock_reason=?,last_failure_at=?,
      last_failure_ip=?,updated_at=? WHERE user_id=?`,
      [failures15m, start15m, failures24h, start24h, lockedUntil || null, lockReason,
...
async function unlock(userId) {
  const result = await query(`UPDATE account_login_security SET failures_15m=0,window_15m_started_at=0,
    failures_24h=0,window_24h_started_at=0,locked_until=NULL,lock_reason=NULL,updated_at=?
...
async function listLocked(now = Date.now()) {
  return query(`SELECT s.user_id,u.email,u.name,s.failures_15m,s.failures_24h,s.locked_until,
      s.lock_reason,s.last_failure_at,s.updated_at,w.name AS workspace_name
...
module.exports = { status, recordFailure, clearAfterSuccess, unlock, listLocked };
