...
router.post('/nodes', async (req, res) => {
    ...
    try {
        ...
        // --- Sincronizar nodos con SQLite en segundo plano ---
        try {
            const db = await getDb();
            await db.run('BEGIN TRANSACTION');
            // Solo insertamos/actualizamos para no perder historial si MikroTik no responde a tiempo
            for (const node of nodes) {
                await db.run('INSERT INTO nodes (id, data) VALUES (?, ?) ON CONFLICT(id) DO UPDATE SET data = excluded.data', [node.id, JSON.stringify(node)]);
            }
            await db.run('COMMIT');
        } catch (dbErr) {
            console.error('[DB] Error sincronizando nodos:', dbErr.message);
        }
