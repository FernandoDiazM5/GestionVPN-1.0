...
function normalizeEd25519Key(value, kind) {
  try {
    const key = value?.type === kind ? value : kind === 'private' ? crypto.createPrivateKey(value) : crypto.createPublicKey(value);
...
function signLicense(payload, { keyId, privateKey }) {
  ...
  const encodedHeader = base64url(stableJson({ alg:'EdDSA', kid:keyId, typ:'JPL' }));
...
function verifyLicense(token, { publicKeys, revokedKeyIds = [], expectedInstanceId, now = new Date() }) {
  ...
  try { header=JSON.parse(fromBase64url(parts[1])); payload=JSON.parse(fromBase64url(parts[2])); }
  ...
  if (!crypto.verify(null, Buffer.from(parts.slice(0, 3).join('.')), normalizeEd25519Key(publicKey, 'public'), fromBase64url(parts[3]))) throw new Error('LICENSE_SIGNATURE_INVALID');
  ...
  if (current + CLOCK_TOLERANCE_SECONDS < payload.nbf) return { valid:false, state:'NOT_YET_VALID', payload, keyId:header.kid };
  if (current <= payload.exp + CLOCK_TOLERANCE_SECONDS) return { valid:true, state:'ACTIVE', payload, keyId:header.kid };
  if (current <= payload.graceUntil + CLOCK_TOLERANCE_SECONDS) return { valid:true, state:'OFFLINE_GRACE', payload, keyId:header.kid };
