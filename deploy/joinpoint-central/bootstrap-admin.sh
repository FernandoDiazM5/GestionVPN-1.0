...
docker compose --env-file "$root/config/compose.env" -f "$root/bundle/deploy/joinpoint-central/compose.yaml" \
  run --rm -T --entrypoint node \
  -e CONTROL_ADMIN_BOOTSTRAP_EMAIL -e CONTROL_ADMIN_BOOTSTRAP_NAME -e CONTROL_ADMIN_BOOTSTRAP_PASSWORD \
  central control-plane/src/scripts/bootstrapAdmin.js
